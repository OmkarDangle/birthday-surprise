...
And one more things - never stop laughing. Your laugh is honestly one of the nicest things about you - it always makes me smile. 😊✨
...
noteMessage:
"You mean more to me than words can say. Wishing you endless love and happiness, always. 💛",
...
if (noteText) {
    ...
    noteText.innerHTML = CONFIG.noteMessage.replace(
        /always/i,
...
photoTags.forEach((tag, i) => {
    ...
    if (tag) tag.textContent = CONFIG.photoNotes[i] || "";
...
function buildCalendar(monthName, day) {
    ...
    if (calMonth) calMonth.textContent = monthName;

    if (!calGrid || monthIndex === -1) return;
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        cell.className = "cal-cell" + (d === Number(day) ? " highlight" : "");
...
buildCalendar(CONFIG.birthdayMonth, CONFIG.birthdayDay);
...
/*==========================================================
        SCRIPT.JS PART 3
        Cake • Celebration • Petals • Ending
==========================================================*/
...
function resetExperience(){
    ...
    // back to the very first scene

    currentScene=1;

    showScene(1);

    // reset the gallery so no card is left centered/flipped

    closeCenteredCard();

    // reset the intro typing effect

    typingIndex=0;

    if(typingText) typingText.innerHTML="";

    // reset the gift box back to its unopened state
    ...
    // reset the letter so it can type out fresh next time
...
if(watchAgainBtn){
    ...
    watchAgainBtn.addEventListener("click",resetExperience);
